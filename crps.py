...
as an ensemble or thresholded probabilities. This code is from the Bureau's internal
verification library.

Copied directly from https://github.com/btrotta-bom/aifs-improver/blob/main/crps.py
...
def integral_below(x0, x1, y0, y1):
    """Integral of squared distance between line y=0 and
    straight line joining (x0, y0), (x1, y1)"""
    return (x1 - x0) * (y1**2 + y1 * y0 + y0**2) / 3.0


@jit
def integral_above(x0, x1, y0, y1):
    """Integral of squared distance between between line y=1 and
    straight line joining (x0, y0), (x1, y1)"""
    return (x1 - x0) * ((1 - y0) ** 2 + (1 - y0) * (1 - y1) + (1 - y1) ** 2) / 3.0
...
def crps_exact(
    ...
    Args:
        ...
        forecasts: DataArray containing either REALIZATION_DIM or THRESHOLD_DIM dimension
    ...
    Returns:
        DataArray with same dimensions as forecasts, except for REALIZATION_DIM or THRESHOLD_DIM,
        and dimensions listed in dim
    """

    if REALIZATION_DIM in forecasts.dims:
        crps_dim = "realization"
    elif THRESHOLD_DIM in forecasts.dims:
        crps_dim = "threshold"
    else:
        raise ValueError(
            f"forecasts must contain '{REALIZATION_DIM}' or '{THRESHOLD_DIM}' dimension"
        )
    ...
    if crps_dim == REALIZATION_DIM:
        ...
        if "long_name" not in forecasts.attrs:
            warn("long_name is missing; assuming thresholding type is below")
        ...
        res = xr.apply_ufunc(
            ...
            observations,
            cdf_forecasts,
            cdf_forecasts[THRESHOLD_DIM].values,
            input_core_dims=[[], [THRESHOLD_DIM], []],
            dask="allowed",
            output_dtypes=[float],
            keep_attrs=keep_attrs,
        )
